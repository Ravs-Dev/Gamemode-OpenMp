...
public OnPlayerConnect(playerid)
{
	...
	if (IsPlayerUsingOfficialClient(playerid))
        pData[playerid][isOfficialClient] = true;
...
public OnPlayerExitVehicle(playerid, vehicleid)
{
	...
	{
		if((gettime() - pvData[vehicleid][vCooldownSave] < 20)) return SendClientMessage(playerid, COLOR_WHITE, "Delay 20 detik untuk save");
		pvData[vehicleid][vCooldownSave] = gettime();
		GetVehiclePos(vehicleid, pvData[vehicleid][vPos][0], pvData[vehicleid][vPos][1], pvData[vehicleid][vPos][2]);
		GetVehicleZAngle(vehicleid, pvData[vehicleid][vPos][3]);
...
{
	new msg[144];
	format(msg, sizeof(msg), "%s [%d] : %s", pData[playerid][pName], playerid, text);
...
public OnPlayerModelSelection(playerid, response, listid, modelid)
{
	if(listid == m_SkinSpawn) 
	{
		if(response)
		{
			...
			new query[288];
			mysql_format(g_SQL, query, sizeof(query), "INSERT INTO player (username, ucp) VALUES ('%e', '%e')", pData[playerid][pName], pData[playerid][pUCP]);
